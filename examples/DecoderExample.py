...
def main(files):
    ...
    fig, axes = plt.subplots(4, examples + 1, layout = 'constrained')

    axes[0,0].axis('off')

    for i in range(examples):

        airfoilProperties = np.loadtxt(rootPath + featureExtension + '/' + targetFiles[i])

        axes[0, i+1].text(0, 0, 'Airfoil #' + str(targetFiles[i]).rsplit('_')[1] + '\n' + 'Mach Number: ' + f'{airfoilProperties[-3]:.2f}' + '\n' + 'A.O.A.: ' + f'{airfoilProperties[-3]:.2f}' + '\n' + 'R.E. ' + f'{airfoilProperties[-1]:.4f} * 10^6', size = 12)
        axes[0, i+1].axis('off')

        c1 = axes[1, i+1].contourf(xArray[i], yArray[i], groundTruthLabelsExtended[i], vmin = groundTruthMin, vmax = groundTruthMax, levels = np.linspace(groundTruthMin, groundTruthMax, 101), cmap = colormaps["coolwarm"])
        ...
        c2 = axes[2, i+1].contourf(xArray[i], yArray[i], predictionLabelsExtended[i], vmin = groundTruthMin, vmax = groundTruthMax, levels = np.linspace(groundTruthMin, groundTruthMax, 101), cmap = colormaps["coolwarm"])
        ...
        c3 = axes[3, i+1].contourf(xArray[i], yArray[i], error, vmin = 0.0, vmax = 1.0, levels = np.linspace(0, 1, 101), cmap = colormaps["binary"])
        c3.set_edgecolor("face")
        axes[3, i+1].set_title("{:.3f}".format(l2Error), y = -0.01, fontsize = 12)
        axes[3, i+1].axis('off')
        axes[3, i+1].set_xlim([-0.5,1.5])
        axes[3, i+1].set_ylim([-1,1])

        airfoilGeometry = Polygon(coords, facecolor='none', edgecolor='blue', linewidth = 1)
        axes[3, i+1].add_patch(airfoilGeometry)

    axes[1,0].text(1, 0.5, 'Ground Truth', ha = 'center', va = 'center', size = 18)
    axes[1,0].axis('off')
    axes[2,0].text(1, 0.5, 'Reconstruction', ha = 'center', va = 'center', size = 18)
    axes[2,0].axis('off')
    axes[3,0].text(1, 0.5, "Absolute Error\n"
                    r'$\vert{x_{\text{real}} - x_{\text{pred}}}\vert$', ha = 'center', va = 'center', size = 18)
    axes[3,0].text(1, 0, r'$\text{L}_2$ Error'
                   "\n"
    ...
    cb = plt.colorbar(c1, ax=axes[[1, 2], examples], shrink = 0.5, format=tkr.FormatStrFormatter('%.2f'))
    cb.ax.tick_params(labelsize = 12)
    cb.set_ticks(ticks = [groundTruthMin, groundTruthMin/2, 0, groundTruthMax/2, groundTruthMax])

    cb = plt.colorbar(c3, ax=axes[3, examples], format=tkr.FormatStrFormatter('%.2f'))
...
if __name__ == "__main__":
    ...
    requestTarget = '1Eobse9S__wR9TmdvyMx7WICHYyyI16Af'

    fileName = 'DecoderExample'
    ...
    filesToLoad = [
        'airfoil_238_G2_A_L0_case_13_000_surf.csv',
        'airfoil_362_G2_A_L0_case_8_000_surf.csv',
        'airfoil_491_G2_A_L0_case_1_000_surf.csv',
        'airfoil_1441_G2_A_L0_case_4_000_surf.csv',
        'airfoil_31151_G2_A_L0_case_8_000_surf.csv',
    ]
